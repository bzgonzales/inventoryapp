...
class Request extends Controller
{
	...
	public function other($var = false){
		...
		echo $model->test('string to touched');

	}

	function login_verification(){

		//if(prevent_CSRF() == TRUE && !$_POST) { echo 'show_404()'; }
		...
		if ($orders->records_total > 0):
			foreach ($orders->records_filtered as $key => $row):
				...
				if($usertype!='customer'){
					$action = '<a href="#" class="call-order-modal btn btn-default btn-sm" data-toggle="modal" data-target="#order-modal" data-order-id="' . $row['order_id']. '"><i class="fa fa-edit"></i></a>';
				}else{
					$action = '';
				}
	...
	function save_order_data(){
		...
		if($_POST['orderid']=='add'){
			$updated = $this->model->insert_order($_POST);
		}else{
			$updated = $this->model->save_order($_POST);
		}
